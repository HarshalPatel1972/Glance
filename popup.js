document.addEventListener("DOMContentLoaded", () => {
  ...
  function loadAll() {
    chrome.storage.session.get({ activeSnips: [] }, (sessionRes) => {
      const activeSnips = Array.isArray(sessionRes.activeSnips)
        ? sessionRes.activeSnips
        : [];
    ...
    chrome.storage.local.get({ savedSnips: [] }, (result) => {
      ...
      renderGrid(savedContainer, snips, (snip) => {
        injectSnipToCurrentTab(snip.image, () =>
          showPopupToast("Snip restored!"),
        );
      });
    });
  }
  ...
  document.getElementById("load-workspace").addEventListener("click", () => {
    chrome.storage.local.get({ savedWorkspace: [] }, (res) => {
      chrome.storage.session.set(
        { activeSnips: res.savedWorkspace },
        async () => {
          chrome.runtime.sendMessage({
            action: "update_badge",
            count: res.savedWorkspace.length,
          });

...
async function sendWithInjection(tabId, message) {
  ...
  const missingReceiver =
    result.error && result.error.includes("Receiving end does not exist");
...
chrome.commands.onCommand.addListener(async (command) => {
  if (command === "trigger_snip") {
    try {
      ...
      const result = await sendWithInjection(tab.id, {
        action: "activate_snip",
      });
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  if (request.action === "capture_area") {
    ...
    chrome.tabs.captureVisibleTab(null, { format: "png" }, (dataUrl) => {
      if (chrome.runtime.lastError) {
        console.error(
          "[Glance BG] captureVisibleTab error:",
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === "get_active_snips") {
    chrome.storage.session.get({ activeSnips: [] }, (result) => {
      ...
      sendResponse({
        ok: true,
        activeSnips: Array.isArray(result.activeSnips)
          ? result.activeSnips
          : [],
      });
    });
  ...
  if (request.action === "set_active_snips") {
    const activeSnips = Array.isArray(request.activeSnips)
      ? request.activeSnips
      : [];
